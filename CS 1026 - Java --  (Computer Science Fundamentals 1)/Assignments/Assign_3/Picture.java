...
////////////////////////PART B METHODS///////////////////
  ...
  private int topX(Color c)
  {
    ...
    for (int x=this.getWidth()-1; x>= 0; x--)                      //loops through the columns (horizontally) starting at the very right and going left
    {
      for (int y= 0; y< this.getHeight(); y++)                     //loops through the rows (vertically)
      {
        ...
        if (sourceColour.equals(c))                                //if the colour of the current pixel matches the colour c (the parameter passed)
        { 
          xCoordinate = x;                                //sets the value of xCoordinate to be the current value of x. At the end of the loop it will return the last 
        }                                                     //value of x and since it started from the right, the last value will be the 
                                                              //left (left most value, first pixel in X direction)
      }//end inner loop
    }//end outer loop
  ...
  private int bottomX(Color c)
  {
    ...
    for (int x=0; x< this.getWidth(); x++)                         //loops through the columns (horizontally)
    {
      for (int y= 0; y< this.getHeight(); y++)                     //loops through the rows (vertically)
      {
        ...
        {
          xCoordinate = x;                                //sets the value of xCoordinate to be the current value of x. At the end of the loop it will return the last 
        }                                                     //value of x and since it started from the left, the last value will be the 
                                                              //right (right most value, last pixel in X direction)
      }//end inner loop
    }//end outer loop   
    return xCoordinate;            //returns the value of xCoordinate  
  }
  ...
  public Picture cropPicture(Color c)
  {
    ...
    int topX = this.topX(c);                                    //integer variabes to store the returned values from the 4 invoked methods
    int bottomX = this.bottomX(c);
    int topY = this.topY(c);
    int bottomY = this.bottomY(c);
    
    Picture targetPicture = new Picture ((bottomX - topX), (bottomY - topY));     //creates a new picture with dimencions equal to the range between the first and last pixel in the x and y directions
    Pixel sourcePixel = null;                                   //creates two pixel objects that are used to get pixels from the source
    Pixel targetPixel = null;                                   // pic to be copied to the target pic
            
    
    for (int x = topX; x < bottomX && x <this.getWidth(); x++)               //loops through the columns from the first pixel to the last pixel that match the colour c.
    {
      for (int y =topY; y < bottomY && y<this.getHeight(); y++)              //loops through the rows from the first pixel to the last pixel that match the colour c.
      {
        sourcePixel = this.getPixel(x,y);                                    //gets the pixel from the original image at location (x,y)
        targetPixel = targetPicture.getPixel(x - topX,y-topY);   //gets the target pixel. x-topX and y-topY makes it so that the target pixels starts at (0,0) (the whole thing is shifted left by topX and up by topY)
       
        targetPixel.setColor(sourcePixel.getColor());                        /*sets the colour of the current target pixel to match the colour of the corresponding pixel
                                                                                   from the source image (the picture object that invokes this method)*/
      }//end inner loop
    }//end outer loop
